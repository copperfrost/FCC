...
bool* bin_parse(const char* str, const unsigned int expected_length) {
	if (!str) {
	...
	for (length = 0; str[length] != '\0'; length++) {
		...
		if (length + 1 > MAX_INPUT_LENGTH) {
			printf("Fatal error: the definition vector is too long to parse.\n");
			exit(EXIT_FAILURE);
		}
	}
...
int main(int argc, char* argv[]) {
	...
	int* walsh_hadamard_coeffs = (int*)calloc(get_defvector_length(N), sizeof(int));
	...
	for (unsigned int k = 0; k < get_defvector_length(N); k++) {
		bool* x = (bool*)calloc(N, sizeof(bool));
		for (unsigned int i = 0; i < get_defvector_length(N); i++) {
			walsh_hadamard_coeffs[k] += ((get_scalar_product(b, x, N) ^ funcdef[bin_to_int(x, N)]) ? -1 : 1);
			printf(".");
			if (can_increment_vector(x, N))
				increment_vector(x, N);
		}
		if (can_increment_vector(b, N))
			increment_vector(b, N);
		free(x);
	}
	...
	printf("\n\nWalsh-Hadamard coefficients for func are:\n\n");
	ct = (bool*)calloc(N, sizeof(bool));
	for (unsigned int i = 0; i < get_defvector_length(N); i++) {
		printf("z(");
		print_vector(ct, N);
		printf(") = %d\n", walsh_hadamard_coeffs[i]);
		if (can_increment_vector(ct, N))
			increment_vector(ct, N);
	}
	printf("\nPress Enter to quit.\n\n");
